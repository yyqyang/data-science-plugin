---
name: polars
description: "Polars expression API for high-performance DataFrame operations, lazy evaluation, joins, aggregations, and I/O. Use as a parallel alternative to pandas-pro when working with large datasets or generating Polars code for data loading, manipulation, or profiling in /ds:eda, /ds:preprocess, or /ds:experiment."
---
...
- Working with large datasets (10M+ rows or >100MB) where lazy evaluation provides significant performance benefits
- Loading, filtering, and transforming tabular data with Polars expression API
- Building query pipelines with lazy evaluation and automatic query optimization
- Reading/writing Parquet, CSV, JSON with Arrow-native I/O
- Performing group_by aggregations and window functions with `over()`
- Joining and concatenating DataFrames with Polars join semantics
- Migrating existing pandas code to Polars for performance gains

## Do NOT Use This Skill

- For pandas API patterns (indexing, groupby, merge) -- use `pandas-pro`
- For pipeline-oriented preprocessing with step tracking -- use `data-preprocessing` (pandas-based)
- For in-model preprocessing inside sklearn Pipelines -- use `scikit-learn`
- For statistical modeling with formula API -- use `statsmodels` (expects pandas)
